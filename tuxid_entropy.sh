...
# add_signal_name()
#
# Aulixiary function that add signal names to the $signals_names array
# avoiding duplicates
#
# Arguments:
...
# generate_tables()
#
# Present output data to the user based on the selected format defined
# by the input flag --format (either table, csv or json)
#============================================================================
generate_tables() {

    # Extract all hashes present in a JSON file group by signal names
    IFS=$signal_separator
    ...
    if [ "$output_format" = "table" ]; then
        ...
        for signal_name in $signals_names; do
            ...
            printf "        \"unique\": %d,\n" "\"$unique_hashes\""
            printf "        \"collisions\": %d,\n" "\"$collisions\""
            printf "        \"entropy\": %.4f" "\"$normalized_entropy\""
            printf "      }"
        done
        printf "\n    ]"
        echo "}"
