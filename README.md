...
| Root Filesystem Available Disk Space 	| volatile           	    | Yes              	    | Moderate (10–15 bits)     | local           	    | local root                | df -h \| grep \\/$ \| tr -s ' ' \| cut -d' ' -f4                                      |
| Root Filesystem Used Disk Space      	| volatile           	    | Yes              	    | Moderate (10–15 bits)     | local           	    | local root                | df -h \| grep \\/$ \| tr -s ' ' \| cut -d' ' -f3                                      |
| AC Power State                       	| volatile           	    | Yes              	    | Extremely Low (1 bit)     | local           	    | local root                | grep '(Charging\\\|Discharging)' /sys/class/power_supply/BAT0/status                  |
...
| Linux session ID  | Volatile (per session)    | No                    | Medium (64 bits)      | User                  | not resettable (generated by OS)          | cat /proc/self/sessionid              |
| Linux user ID     | Permanent                 | Yes                   | Low (~16 bits)        | User                  | local root                                | id -u                                 |
